...
#include "Camera/DynamicCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/CameraPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "DynamicCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(DynamicCameraMode_ThirdPerson)

namespace DynamicCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UDynamicCameraMode_ThirdPerson::UDynamicCameraMode_ThirdPerson()
{
...
void UDynamicCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UDynamicCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UDynamicCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UDynamicCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IDynamicCameraAssistInterface* TargetControllerAssist = Cast<IDynamicCameraAssistInterface>(TargetController);

	IDynamicCameraAssistInterface* TargetActorAssist = Cast<IDynamicCameraAssistInterface>(TargetActor);
	...
	IDynamicCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IDynamicCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IDynamicCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IDynamicCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UDynamicCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IDynamicCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	for (int32 RayIdx = 0; RayIdx < NumRaysToShoot; ++RayIdx)
	{
		...
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(DynamicCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UDynamicCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UDynamicCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
